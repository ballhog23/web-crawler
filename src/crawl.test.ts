...
test("getImagesFromHTML relative svg", () => {
  const inputURL = "https://blog.boot.dev";
  const inputBody = `<html><body><img src="./images/svg/logo.svg" alt="Logo"></body></html>`;
  const actual = getImagesFromHTML(inputBody, inputURL);
  const expected = ["https://blog.boot.dev/images/svg/logo.svg"];
  expect(actual).toEqual(expected);
});
...
// test("extractPageData multiple relative and absolute", () => {
//   const inputURL = "https://blog.boot.dev";
//   const inputBody = `
//     <html><body>
//       <h1>Test Title</h1>
